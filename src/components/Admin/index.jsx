...
function Admin() {
  ...
  const searchHandler = (e) => {
    ...
    setFilterData(
      dataList.filter((x) => {
        return x.namaKepalaBidang.toLowerCase().includes(value.toLowerCase());
      })
    );
  };
  ...
  const sortHandler = (id) => {
    if (id === "Kepala Bidang") {
      const sortedNew = [...filterData].sort((a, b) => a.namaKepalaBidang.localeCompare(b.namaKepalaBidang));
      setFilterData(sortedNew);
    } else if (id === "Kepala Seksi") {
      const sortedNew = [...filterData].sort((a, b) => a.namaKepalaSeksi.localeCompare(b.namaKepalaSeksi));
      setFilterData(sortedNew);
    } else if (id === "Nama Program") {
      const sortedNew = [...filterData].sort((a, b) => a.namaProgram.localeCompare(b.namaProgram));
      setFilterData(sortedNew);
    } else if (id === "Sub Kegiatan") {
      const sortedNew = [...filterData].sort((a, b) => a.namaSubKegiatan.localeCompare(b.namaSubKegiatan));
      setFilterData(sortedNew);
    } else if (id === "Rincian Sub Kegiatan") {
      const sortedNew = [...filterData].sort((a, b) => a.rincianSubKegiatan.localeCompare(b.rincianSubKegiatan));
      setFilterData(sortedNew);
    } else if (id === "No Rekening") {
      const sortedNew = [...filterData].sort((a, b) => a.noRekening.localeCompare(b.noRekening));
      setFilterData(sortedNew);
    } else if (id === "Paket Pekerjaan") {
      const sortedNew = [...filterData].sort((a, b) => a.paketPekerjaan.localeCompare(b.paketPekerjaan));
      setFilterData(sortedNew);
    } else if (id === "Nilai Ditagih") {
      const sortedNew = [...filterData].sort((a, b) => a.nilaiDitagih.localeCompare(b.nilaiDitagih));
      setFilterData(sortedNew);
    } else if (id === "Tahun Anggaran") {
      const sortedNew = [...filterData].sort((a, b) => a.tahunAnggaran.localeCompare(b.tahunAnggaran));
      setFilterData(sortedNew);
    }
  };
            ...
            <div style={{ border: "1px solid gray" }} className="mt-3">
              ...
              <div className="px-3 pb-2">
                <div className="d-flex align-items-center justify-content-between w-100">
                  <input onChange={(e) => searchHandler(e)} className="px-2" placeholder="Search"></input>
                ...
                <table className="w-100 border mt-3">
                  <tr>
                    {tableList.map((x) => {
                      return (
                        <th key={x}>
                          <div onClick={() => sortHandler(x)} style={{ cursor: "pointer" }} className="d-flex align-items-center">
                            {x}
